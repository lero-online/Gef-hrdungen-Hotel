...
def dump_excel(assess: Assessment) -> bytes:
    ...
    with pd.ExcelWriter("Gefaehrdungsbeurteilung.xlsx", engine="openpyxl") as writer:
    ...
    with open("Gefaehrdungsbeurteilung.xlsx", "rb") as f:
        return f.read()
...
def preload_template(assess: Assessment):
    # Küche ist Kern; andere Bereiche optional auswählbar im UI
...
with st.sidebar:
    ...
    assess.company = st.text_input("Unternehmen", assess.company)
    assess.location = st.text_input("Standort", assess.location)
    assess.created_by = st.text_input("Erstellt von", assess.created_by)
    assess.created_at = st.text_input("Erstellt am (ISO)", assess.created_at)

    st.markdown("---")
    st.subheader("Risikomatrix (5×5)")
    thr = assess.risk_matrix_thresholds.get("thresholds", [6, 12, 16])
    low = st.number_input("Grenze niedrig (≤)", min_value=2, max_value=10, value=int(thr[0]))
    mid = st.number_input("Grenze mittel (≤)", min_value=low+1, max_value=16, value=int(thr[1]))
    high = st.number_input("Grenze hoch (≤)", min_value=mid+1, max_value=24, value=int(thr[2]))
    assess.risk_matrix_thresholds["thresholds"] = [low, mid, high]

    st.markdown("---")
    st.subheader("Export / Speicher")
    if st.button("📥 JSON sichern (Download unten aktualisieren)"):
        st.session_state["json_blob"] = as_json(assess)
    json_blob = st.session_state.get("json_blob", as_json(assess))
    st.download_button("⬇️ Download JSON", data=json_blob, file_name="gefaehrdungsbeurteilung.json", mime="application/json")

    excel_bytes = dump_excel(assess)
    st.download_button("⬇️ Download Excel", data=excel_bytes, file_name="Gefaehrdungsbeurteilung.xlsx", mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")

    st.markdown("---")
    st.subheader("JSON laden")
    up = st.file_uploader("Bestehende Beurteilung (.json)", type=["json"])
    if up is not None:
...
with tabs[0]:
    ...
    assess.scope_note = st.text_area("Umfang / Arbeitsbereiche / Beteiligte (SiFa, Betriebsarzt, BR, Führungskräfte, Beschäftigte)", value=assess.scope_note, height=140)
...
with tabs[1]:
    ...
    with colL:
        ...
        with st.expander("➕ Gefährdung hinzufügen"):
            ...
            area = col1.selectbox("Bereich", sorted(list(TEMPLATES.keys()) + ["Sonstiges"]))
            activity = col2.text_input("Tätigkeit")
            hazard_txt = st.text_input("Gefährdung")
            sources = st.text_input("Quellen/Einwirkungen (durch ; trennen)")
            existing = st.text_input("Bestehende Maßnahmen (durch ; trennen)")
    ...
    with colR:
        ...
        sel_id = st.selectbox("Gefährdung auswählen (ID)", options=["--"] + ids, index=0)
        if sel_id != "--":
            hz = next(h for h in assess.hazards if h.id == sel_id)
            hz.area = st.selectbox("Bereich", options=list(TEMPLATES.keys()) + ["Sonstiges"], index=(list(TEMPLATES.keys()) + ["Sonstiges"]).index(hz.area) if hz.area in (list(TEMPLATES.keys()) + ["Sonstiges"]) else 0)
            hz.activity = st.text_input("Tätigkeit", value=hz.activity)
            hz.hazard = st.text_input("Gefährdung", value=hz.hazard)
            src = st.text_area("Quellen/Einwirkungen", value="; ".join(hz.sources))
            hz.sources = [s.strip() for s in src.split(";") if s.strip()]
            ex = st.text_area("Bestehende Maßnahmen", value="; ".join(hz.existing_controls))
...
with tabs[2]:
    ...
    with colA:
        sel = st.selectbox("Gefährdung auswählen", options=[f"{h.id} – {h.area}: {h.hazard}" for h in assess.hazards])
        hz = assess.hazards[[f"{h.id} – {h.area}: {h.hazard}" for h in assess.hazards].index(sel)]
        hz.prob = st.slider("Eintrittswahrscheinlichkeit (1 = sehr selten … 5 = häufig)", 1, 5, hz.prob)
        hz.sev = st.slider("Schadensschwere (1 = gering … 5 = katastrophal)", 1, 5, hz.sev)
        ...
        hz.documentation_note = st.text_area("Beurteilungs-/Dokumentationshinweis", value=hz.documentation_note)
...
with tabs[3]:
    ...
    sel = st.selectbox("Gefährdung für Maßnahmen wählen", options=[f"{h.id} – {h.area}: {h.hazard}" for h in assess.hazards])
    hz = assess.hazards[[f"{h.id} – {h.area}: {h.hazard}" for h in assess.hazards].index(sel)]

    with st.expander("➕ Maßnahme hinzufügen"):
        title = st.text_input("Maßnahme")
        stop = st.selectbox("STOP(+Q)", STOP_LEVELS, index=0)
        responsible = st.text_input("Verantwortlich")
        due = st.date_input("Fällig am", value=date.today()+relativedelta(months=1))
...
with tabs[5]:
    ...
    hz.last_review = st.date_input("Datum der Überprüfung", value=date.today()).isoformat()
    hz.reviewer = st.text_input("Prüfer/in", value=hz.reviewer)

# 7 Dokumentation
with tabs[6]:
    st.subheader("7) Ergebnisse dokumentieren")
    assess.documentation_note = st.text_area("Dokumentationshinweis (welche Unterlagen, wo abgelegt, Versionierung)", value=assess.documentation_note, height=120)
...
with tabs[7]:
    ...
    assess.next_review_hint = st.text_area("Anlässe/Fristen (z. B. jährliche Überprüfung, nach Unfällen/Beinaheunfällen, bei Änderungen von Verfahren/Organisation/Arbeitsmitteln)", value=assess.next_review_hint, height=100)
...
with tabs[8]:
    ...
    assess.measures_plan_note = st.text_area("Projekt-/Maßnahmenplan (kurz)", value=assess.measures_plan_note)
